%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Matrix unwrap algorithm adapted from                                         %
...
function res_img = unwrap_phase(img,deg)



    [~,~,Ny, Nx] = size(img);

    gx = pagedet(img(:,:,:,2:end),img(:,:,:,1:end-1));
    gx = squeeze(gx) > 0;
    gy = pagedet(img(:,:,2:end,:),img(:,:,1:end-1,:));
    gy = squeeze(gy) > 0;

    

    deg = wrapToPi(imag(deg(1,1,:,:)-deg(2,2,:,:)));


    gx = [gather(gx) nan(Ny,1)];
    gy = [gather(gy); nan(1,Nx)];




    % get the reliability
    reliability = get_reliability(img,deg); % (Ny,Nx)
    ...
    for i = num_nan+1 : length(edge_sort_idx)
        ...
        if g
            if res_img(idx1)*res_img(idx2) < 0
                res_img(pix_idxs) = res_img(pix_idxs)*(-1);
            end
        else 
            if res_img(idx1)*res_img(idx2) > 0
                res_img(pix_idxs) = res_img(pix_idxs)*(-1);
    ...
    res_img = reshape(res_img,1,1,Ny,Nx);

 
end

function rel = get_reliability(img,deg)
    rel = zeros(size(img,3),size(img,4));
    deg = deg(:,:,2:end-1, 2:end-1);



    % calculate the difference
    lie = @(x,y) pagemtimes(x,y)-pagemtimes(y,x);
    lie2 =  @(x,y,z) pagenorm(lie(lie(x,y),lie(y,z)),"fro");
    H  = lie2(img(:,:,1:end-2, 2:end-1), img(:,:,2:end-1, 2:end-1), img(:,:,3:end  , 2:end-1));
    V  = lie2(img(:,:,2:end-1, 1:end-2), - img(:,:,2:end-1, 2:end-1), img(:,:,2:end-1, 3:end));
    D1 = lie2(img(:,:,1:end-2, 1:end-2), - img(:,:,2:end-1, 2:end-1), img(:,:,3:end  , 3:end));
    D2 = lie2(img(:,:,1:end-2, 3:end), - img(:,:,2:end-1, 2:end-1), img(:,:,3:end  , 1:end-2));

    rel(2:end-1, 2:end-1) = squeeze(abs(deg).^2./sqrt(H+V+D1+D2));

    % assign all nan's in rel with non-nan in img to 0
    % also assign the nan's in img to nan
    rel(isnan(rel) & ~isnan(squeeze(sum(img,[1,2])))) = 0;
    rel(isnan(squeeze(sum(img,[1,2])))) = nan;
end
